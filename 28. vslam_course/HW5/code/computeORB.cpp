...
// global variables
string first_file = "/home/lwl/workspace/3rd-test-learning/28. vslam_course/HW5/code/1.png";
string second_file = "/home/lwl/workspace/3rd-test-learning/28. vslam_course/HW5/code/2.png";
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        // 边缘的check
        if(kp.pt.x+half_patch_size>=image.cols ||
           kp.pt.y+half_patch_size>=image.rows ||
           kp.pt.x-half_patch_size<0 ||
           kp.pt.y-half_patch_size<0)
           continue;
        ...
        for(int v=-half_patch_size;v<=half_patch_size;v++)
        {
            ...
            {
                m10 += u*center[u+v*step];
                m01 += v*center[u+v*step];
            }
        }
        kp.angle = cvFastArctan(m01,m10);
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    ...
    for (auto &kp: keypoints) {
        ...
        float angle = kp.angle*factorPI;
        if(kp.angle >4)
        {
            std::cout<<"the angle is radias?"<<std::endl;
        }    
        ...
        int * pattern = ORB_pattern;

        for (int i = 0; i < 256; i++,pattern+=4) {
            // START YOUR CODE HERE (~7 lines)
            int t0,t1;
            bool val;
