...
bool ICPRegistrationSingle(pcl::PointCloud<pcl::PointXYZ>::Ptr t_pc_ptr,pcl::KdTreeFLANN<pcl::PointXYZ>::Ptr kdtree_ptr,
                              ...
                              bool & finish,
                              float max_dis_squared = 10,
                              float min_err2 = 2e-6) // 感觉重点还是在这个参数上面 决定优化次数
    {
...
bool LoadTrajectory(std::string  path, vector<Eigen::Vector3d> & trajectory)
{
    trajectory.clear();
    ...
    if(!fin.is_open())
    {
        cerr<<"There is something wrong with the path"<<endl;
        abort();
    }
    while(getline(fin,data_line))
    {
        ...
        trajectory.push_back(Eigen::Vector3d(tx,ty,tz));
    }
}

bool EigenToPCL(const vector<Eigen::Vector3d> & trajectory_eigen, 
                pcl::PointCloud<pcl::PointXYZ> & trajectory_pcl)
{
    trajectory_pcl.clear();
    for(auto & p:trajectory_eigen)
    {
        pcl::PointXYZ pp;
        pp.x = p.x();
        pp.y = p.y();
        pp.z = p.z();
        trajectory_pcl.push_back(pp);
    }
}

bool PCLToEigen(const pcl::PointCloud<pcl::PointXYZ> & trajecotry_pcl,
                vector<Eigen::Vector3d> & trajectory_eigen)
{
    trajectory_eigen.clear();
    for(int i =0;i<trajecotry_pcl.size();i++)
    {
        pcl::PointXYZ pp = trajecotry_pcl[i];
        Eigen::Vector3d  p (pp.x,pp.y,pp.z);
        trajectory_eigen.push_back(p);
    }
}

void DrawTrajectory(vector<Eigen::Vector3d> & poses,
                    vector<Eigen::Vector3d> & gt) {
    if (poses.empty()) {
    ...
    while (pangolin::ShouldQuit() == false) {
        ...
        for (size_t i = 0; i < poses.size() - 1; i++) {
            ...
            glBegin(GL_LINES);
            Eigen::Vector3d & p1 = poses[i],p2 = poses[i + 1];
        ...
        for (size_t i = 0; i < gt.size() - 1; i++) {
            ...
            glBegin(GL_LINES);
            Eigen::Vector3d & p1 = gt[i],p2 = gt[i + 1];
            glVertex3d(p1.x(), p1.y(), p1.z());
            glVertex3d(p2.x(),p2.y(),p2.z());
            glEnd();
        }
...
int main()
{
    ...
    // 轨迹导入
    vector<Eigen::Vector3d> gt,t,transform_t,r;
    pcl::PointCloud<pcl::PointXYZ>::Ptr gt_p(new pcl::PointCloud<pcl::PointXYZ>()),
                                        t_p(new pcl::PointCloud<pcl::PointXYZ>()),
                                        transformed_t_p(new pcl::PointCloud<pcl::PointXYZ>()),
                                        temp_transformed_t_p(new pcl::PointCloud<pcl::PointXYZ>());
    LoadTrajectory("/home/lwl/workspace/3rd-test-learning/28. vslam_course/HW5/code/groundtruth.txt",gt);
    LoadTrajectory("/home/lwl/workspace/3rd-test-learning/28. vslam_course/HW5/code/trajectory.txt",t);
    ...
    transform_test<<
        0.475569   , -0.879562 ,   0.0143053  ,     0.0811
   ,     0.879678  ,   0.475521,   -0.0068145 ,     0.33754
   , -0.000808696  ,  0.0158249,     0.999874 ,    0.095587
   ,         0     ,       0   ,         0    ,        1;


    // 展示轨迹

    pcl::transformPointCloud(*t_p,*transformed_t_p,transform_test.cast<float>());
    pcl::transformPointCloud(*t_p,*temp_transformed_t_p,transform_test.cast<float>());
    // PCLToEigen(*transformed_t_p,transform_t);
    // DrawTrajectory(gt,transform_t);

    kdtree->setInputCloud(gt_p);
    Eigen::Matrix4f iterative_result = Eigen::Matrix4f::Identity();
    bool finish = false;
    for(int i =0;i<10;i++)
    {
        ICPRegistrationSingle(temp_transformed_t_p,kdtree,iterative_result,iterative_result,finish);
        if(finish)
        {
        ...
        pcl::transformPointCloud(*transformed_t_p,*temp_transformed_t_p,iterative_result);
    }

    cout<< "the final estimate result is "<<endl
    <<iterative_result<<endl
    <<"the inverse of "<<endl
    <<iterative_result.inverse();
    
    PCLToEigen(*temp_transformed_t_p,r);
    DrawTrajectory(gt,r);
