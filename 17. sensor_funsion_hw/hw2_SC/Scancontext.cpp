...
std::vector<std::pair<int,float>> SCManager::detectLoopClosure(void)
{
     int loop_id { -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")
    ...
    {
        std::pair<int, float> result {loop_id, 0.0};
        std::vector<std::pair<int,float>> result_array;
        return result_array; // Early return 
    }
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        if( candidate_dist < SC_DIST_THRES )
        {
            ...
            nn_idx = candidate_indexes[candidate_iter_idx];
            float yaw_diff_rad = deg2rad(nn_align * PC_UNIT_SECTORANGLE);
            std::pair<int,float> candidate{nn_idx,yaw_diff_rad};
            result_array.push_back(candidate);
        }
